  ...
  <div class="dashboard">
    ...
    <div class="card">
      ...
      <div v-else class="agendamentos-list">
        <div v-for="ag in agendamentosStore.agendamentosDashboard" :key="ag.id" class="ag-card" :class="ag.status_presenca">
          <div class="ag-header">
            <div>
              <h4>{{ ag.pet_nome }}</h4>
              <p class="cliente">{{ ag.cliente_nome }}</p>
            </div>
<span class="status-badge" :class="`status-${ag.status_presenca}`">{{ ag.status_presenca.toUpperCase() }}</span>
          </div>
